...
function dbToMatch(row: Record<string, unknown>): Match {
  return {
    ...
    ctSetup: (row.ct_setup as Match["ctSetup"]) || "Default",
    ...
    trFinalizacion: (row.tr_finalizacion as Match["trFinalizacion"]) || "Default",
...
function matchToDb(match: Partial<Match> & { recorded_by?: string }) {
  return {
    ...
    ct_setup: match.ctSetup || "Default",
    ...
    tr_finalizacion: match.trFinalizacion || "Default",
