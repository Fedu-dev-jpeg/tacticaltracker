...
import { Bell, CalendarCheck, ClipboardList, Trophy, UserCheck } from "lucide-react";
...
export default function NotificationsCenter({ matches }: { matches: Match[] }) {
  ...
  const urgentCount = notifications.filter((item) => item.severity !== "info").length;
              ...
              {notifications.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full rounded-md border border-border bg-card p-3 text-left transition hover:border-accent/40 hover:bg-accent/5"
                  >
